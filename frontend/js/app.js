  ...
  const views = {
    ...
    messages: {
      ...
      mount({ container, user, params, router }) {
        ...
        function updateUrlForChat(userId) {
          window.history.replaceState(null, '', `${window.location.pathname}${buildHash('messages', { user: userId })}`);
        }

        function renderInbox() {
          if (!conversations.length) {
            inboxList.innerHTML = '<p class="text-sm text-slate-400 p-4">No hay conversaciones aun</p>';
            return;
          }
          ...
          inboxList.innerHTML = conversations.map((conversation) => {
            ...
            const isActive = Number(otherUser.id) === Number(activeChat);
            return `
              <button type="button" class="w-full flex items-start gap-3 p-3 mx-2 ${isActive ? 'bg-slate-100' : 'hover:bg-slate-50'} rounded-xl cursor-pointer transition-colors text-left" data-open-chat="${otherUser.id}">
                ${renderAvatar(otherUser, { sizeClass: 'w-12 h-12', textClass: 'text-white font-bold', showOnline: true })}
                <div class="flex-1 min-w-0">
                  <div class="flex justify-between items-center mb-0.5">
                    <div class="flex items-center gap-1.5 min-w-0">
                      <h3 class="font-bold text-sm text-slate-900 truncate">${escapeHtml(displayName(otherUser))}</h3>
                      <span class="text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full shrink-0" style="background:${userColor(otherUser)}">${escapeHtml(otherUser.faculty || '')}</span>
                    </div>
                  </div>
                  <p class="text-sm text-slate-500 truncate">${escapeHtml(conversation.last_message?.content || '')}</p>
                </div>
              </button>
            `;
          }).join('');
        ...
        function renderMessages(messages) {
          ...
          if (!messages.length) {
            area.innerHTML = '<p class="text-center text-slate-400 text-sm">No hay mensajes. Empieza la conversacion.</p>';
            return;
          }
          ...
          area.innerHTML = messages.map((message) => {
            ...
            if (isMine) {
              return `
                <div class="flex flex-col items-end self-end max-w-[70%]">
                  <div class="bg-[#2B3567] text-white px-5 py-3 rounded-2xl rounded-tr-sm shadow-sm">${nl2br(message.content || '')}</div>
                  <span class="text-[11px] text-slate-500 mt-1 mr-1">${escapeHtml(formatClock(message.created_at))}</span>
                </div>
              `;
            }
            ...
            return `
              <div class="flex flex-col items-start max-w-[70%]">
                <div class="bg-white text-slate-800 px-5 py-3 rounded-2xl rounded-tl-sm shadow-sm border border-slate-100">${nl2br(message.content || '')}</div>
                <span class="text-[11px] text-slate-500 mt-1 ml-1">${escapeHtml(formatClock(message.created_at))}</span>
              </div>
            `;
          }).join('');
        ...
        async function loadConversation(userId, otherUser) {
          activeChat = Number(userId);
          activeUser = otherUser;
          updateUrlForChat(userId);
          renderInbox();

          chatPanel.innerHTML = `
            <div class="flex items-center p-4 bg-white border-b border-slate-200 shrink-0">
              ${renderAvatar(otherUser, { sizeClass: 'w-12 h-12', textClass: 'text-white font-bold', extraClass: 'mr-3', showOnline: true })}
              <div>
                <div class="flex items-center gap-1 flex-wrap">
                  <h2 class="font-bold text-slate-900 text-lg leading-tight">${escapeHtml(displayName(otherUser))}</h2>
                  <span class="text-white text-[10px] font-bold px-2 py-0.5 rounded-full ml-1" style="background:${userColor(otherUser)}">${escapeHtml(otherUser.faculty || '')}</span>
                </div>
                <p class="text-sm text-slate-500">${escapeHtml(careerLabel(otherUser))} · En linea</p>
              </div>
            </div>
            <div class="flex-1 overflow-y-auto p-6 flex flex-col gap-4 custom-scrollbar" id="messages-area">
              <p class="text-center text-slate-400 text-sm">Cargando mensajes...</p>
            </div>
            <div class="p-4 bg-white border-t border-slate-200 shrink-0">
              <div class="flex items-center gap-3">
                <div class="flex-1 relative">
                  <input id="msg-input" class="w-full bg-slate-100 border-none rounded-full py-3 pl-4 pr-10 text-sm focus:ring-1 focus:ring-[#1B2A6B] outline-none" placeholder="Mensaje para ${escapeHtml(displayName(otherUser))}..." type="text"/>
                </div>
                <button id="send-msg-btn" type="button" class="w-12 h-12 rounded-full bg-[#D4A017] flex items-center justify-center text-white hover:bg-[#b88a14] transition-colors shrink-0 shadow-sm">
          ...
          const sendMessage = async () => {
            ...
            input.value = '';
            const result = await ChatAPI.sendMessage(activeChat, content);
            if (!result?.ok) {
              showToast(result?.data?.error || 'Error al enviar el mensaje', 'error');
              return;
            }
            await loadConversation(activeChat, otherUser);
            await loadInbox();
          };
          ...
          const result = await ChatAPI.getConversation(userId);
          renderMessages(getList(result));
        }
        ...
        async function openChatByUserId(userId) {
          ...
          const conversation = conversations.find((item) => Number(item.other_user?.id) === numericUserId);
          let otherUser = conversation?.other_user;

          if (!otherUser) {
            const profileResult = await AuthAPI.getProfile(numericUserId);
            if (!profileResult?.ok) {
              showToast(profileResult?.data?.error || 'No se pudo abrir la conversacion', 'error');
              return;
            }
        ...
        async function loadInbox() {
          const result = await ChatAPI.getInbox();
          conversations = getList(result);
          renderInbox();

          if (activeChat) {
            await openChatByUserId(activeChat);
          }
        }
